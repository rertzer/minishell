...
/*   Updated: 2023/03/16 14:06:42 by rertzer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static void	pp_child_dupfd(t_pipeline *ppl, t_command *cmd, char ***envp);
...
void	pp_child_run(t_pipeline *ppl, t_command *cmd, char ***envp, int i)
{
	...
	{
		if (builtin != 7)
			ms_builtin_run(cmd, envp, 1, 0);
		ms_exit_exit(ppl, envp, cmd->args);
	}
